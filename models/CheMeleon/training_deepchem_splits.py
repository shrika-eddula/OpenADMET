...
def train_one(
    ...
    **kwargs,
    ...
    print(f"  Building pipeline for {target}...")
    pipe = get_prf_pipe(
        extra_transformers=extra_transformers,
        **kwargs,
    )
    
    print(f"  Fitting model on {len(train_idxs)} training samples...")
    # To fit with IndexSplitter and astartes splitter
    pipe.fit(df[SMILES_COL].iloc[train_idxs], df[target].iloc[train_idxs])
    print(f"  Making predictions on {len(val_idxs)} validation samples...")
    ...
    # to fit with ButinaSplitter -----
    # Conversions to Butina where idxs are SMILES
    # df[df[SMILES_COL].isin(train_idxs)]
    # df[df[target].isin(train_idxs)]
    # df[df[SMILES_COL].isin(val_idxs)]
    # df[df[target].isin(val_idxs)]

    
    # pipe.fit(df[df[SMILES_COL].isin(train_idxs)], df[df[target].isin(train_idxs)])
    # print(f"  Making predictions on {len(val_idxs)} validation samples...")
    # val_pred = pipe.predict(df[df[SMILES_COL].isin(val_idxs)])
    # data = {"smiles": df[df[SMILES_COL].isin(val_idxs)].reset_index(drop=True)}
    # data[f"true_{target}"] = df[df[target].isin(val_idxs)].reset_index(drop=True)
    # End ButinaSplitter ------
    ...
    if write_output:
        ...
        print(f"  Creating parity plot for {target}...")
        fig = parity_plot(
    ...
    rmse = root_mean_squared_error(val_df[f"true_{target}"], val_df[f"pred_{target}"])
...
if __name__ == "__main__":
    ...
    # get the data
    data_path = Path("data/datasets/Expansion_Log10_Transformed.csv")
    if data_path.exists():
        _df = pd.read_csv(data_path)
        print(f"Loaded combined dataset from: {data_path}")
    else:
        print(f"Error: Combined dataset not found at {data_path}")
        print(f"Please ensure the file exists at the specified location.")
        exit(1)
    print(f"Loaded data with {len(_df)} compounds and {len(_df.columns)} columns")
    ...
    for _target in TARGETS:
        ...
        print(f"After dropping NaN values for {target}: {len(df)} compounds remaining (dropped {initial_count - len(df)})")
        ...
        if previous_model_paths:
            ...
            print(f"Using {len(previous_model_paths)} previous models as features for {target}")

        # start by hyperparameter optimizing the model
        print(f"Splitting data into train/validation sets...")

        # the following is astartes code
        ...
        #     return_indices=True,
        # )
        # the following is deepchem code
        # IndexSplitter (time based)
        # indexsplitter = dc.splits.IndexSplitter()
        # dataset = dc.data.NumpyDataset(df[SMILES_COL], df.drop(['Unnamed: 0', 'SMILES', 'Molecule Name', 'LogD modifier',
        ...
        # train_dataset, test_dataset = indexsplitter.train_test_split(dataset, frac_train=0.8, seed=42)
        # train_idxs, val_idxs = train_dataset.ids, test_dataset.ids

        # ButinaSplitter (Tanimoto similarity based)
        butinasplitter = dc.splits.ButinaSplitter()
        dataset = dc.data.DiskDataset.from_numpy(X=df["SMILES"],ids=df["SMILES"])
        train_dataset, test_dataset = butinasplitter.train_test_split(dataset, frac_train=0.8, seed=42)
        train_idxs, val_idxs = train_dataset.ids, test_dataset.ids

        print(f"Split data into {len(train_idxs)} training and {len(val_idxs)} validation samples")
        if _target in KNOWN_PARAMS:
            # mock the outcome of the study with known params
            study = SimpleNamespace()
            study.best_params = KNOWN_PARAMS[_target]
            print(f"Using known hyperparameters for {_target}: {study.best_params}")
        else:
            print(f"Starting hyperparameter optimization for {_target} with {TUNING_TRIALS} trials...")
        ...
        print(f"Training validation model for {target}...")
        start_time = time.time()
        train_one(
            df,
            train_idxs,
        ...
        print(f"Validation model trained in {time.time() - start_time:.2f} seconds and saved to {subdir}")

        # using the optimal settings, train a model on the entire dataset for actual submission
        print(f"Training final model for {target} on all {len(df)} samples...")
        start_time = time.time()
        ...
        print(f"Final model trained in {time.time() - start_time:.2f} seconds and saved to {outmodel}")
        previous_model_paths.append(outmodel)
    
    print(f"\n{'='*50}")
    print(f"Training completed for all targets!")
